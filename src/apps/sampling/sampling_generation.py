    ...
    try:
        ...
        with torch.no_grad():
            for batch_idx in range(num_sampled_batches):
                ...
                control_eval = control_emb[:, torch_mask]
                mmd_metric = mmd_loss(sample_eval, truth_eval).item()
                ctrl_to_pert_mmd_metric = mmd_loss(control_eval, truth_eval).item()
                batch_time = time.time() - batch_start_time

                # logger.info(
                #     "Batch %s completed in %.2fs, fake_r2: %s, delta_r2: %s, mmd: %s, ctrl_to_pert_mmd: %s",
                #     batch_idx + 1,
                #     batch_time,
                #     fake_r2_metric,
                #     delta_r2_metric,
                #     mmd_metric,
                #     ctrl_to_pert_mmd_metric,
                # )
